...
try {
    // Pobieramy WSZYSTKIE spektakle od najwcześniejszego do najpóźniejszego
    $stmtHarmonogram = $pdo->query("SELECT id, tytul, opis, data_wystawienia, cena FROM Spektakle ORDER BY data_wystawienia ASC");
...
// Funkcja do tłumaczenia numeru miesiąca na krótki polski skrót (do ikon daty)
...
// Funkcja do pełnych nazw miesięcy (do nagłówków oddzielających sekcje)
...
<head>
    ...
    <style>
        ...
        /* Kontener (Szerokość spójna z index.php - 1200px) */
        ...
        /* Tytuł miesiąca oddzielający bloki spektakli */
        .separator-miesiaca {
            font-size: 24px;
            color: #829356;
...
<body>
    ...
    <div class="kontener-sekcji">
        ...
        <div class="lista-spektakli">
            ...
            <?php else: ?>
                <?php 
                $ostatni_miesiac_rok = ''; // Zmienna śledząca poprzednio wyświetlony miesiąc i rok
                ...
                foreach ($harmonogram as $s): 
                    ...
                    $czas = date('H:i', $timestamp);
                    
                    // Łączymy miesiąc z rokiem, by poprawnie grupować występy z różnych lat
                    $biezacy_miesiac_rok = $nr_miesiaca . '-' . $rok;

                    // Jeżeli obecny miesiąc/rok różni się od ostatnio wyświetlonego - drukujemy separator
                    if ($ostatni_miesiac_rok !== $biezacy_miesiac_rok) {
                        $pelna_nazwa_miesiaca = polskiMiesiacPelny($nr_miesiaca);
                        echo "<div class='separator-miesiaca'>$pelna_nazwa_miesiaca $rok</div>";
